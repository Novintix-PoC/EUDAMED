    ...
    try:
        ...
        if is_yes_no:
            prompt = f"""
                       ...
                       Understand the '{clean_keyword}' and search for it in the files.
                       Interpret the content related to the '{clean_keyword}' and return the correct 'Yes' or 'No' based on that understanding.
                       Ensure the response is based only on the information present in the same file.
            ...
            prompt = f"""
                       ...
                       Understand the meaning of the '{keyword}' and search for it in the file.
                       Extract and return only the exact information directly related to the '{keyword}' from the same file.
                       The model should accurately interpret the content of the file and return only the specific information associated with the '{keyword}' — without any extra text or explanation.
                       ...
                       Do not return the '{keyword}' that appears in the input files itself as the output — only return the associated or descriptive information related to the '{keyword}' in the files.
...
#     """Extract data using ProcessPoolExecutor with model loading per batch"""
#     try:
...
#         # Process batches in parallel, with each batch loading its own model
#         with ProcessPoolExecutor(max_workers=max_workers) as executor:
#             futures = {executor.submit(preprocess_and_extract_batch, batch, keyword, row): batch 
...
#         # Find the first successful result across all batches
#         found_result = next((r for r in batch_results if r["Source File"] != "Not Found"), None)
        
#         if found_result:
#             extracted_data.append(found_result)
#         else:
...
# @st.cache_resource
def create_worker_pool(max_workers=1):
...
def extract_data_from_images(excel_file_path, image_paths, max_workers=1):
...
# def extract_udi(text):
#     """
#     Specialized function to extract UDI numbers
#     """
#     try:
#         # Pattern for UDI numbers
#         udi_patterns = [
#             r'\(01\)(\d+)',  # Basic (01) pattern
#             r'(?<=\(01\))\d+',  # Lookbehind for (01)
#             r'UDI.*?\(01\)(\d+)',  # UDI label followed by number
#             r'(?:^|\n)\s*\(01\)(\d+)'  # (01) at start of line or after newline
#         ]
        
#         for pattern in udi_patterns:
#             matches = re.findall(pattern, text)
#             if matches:
#                 # Return first match that has the expected length for a UDI
#                 for match in matches:
#                     if isinstance(match, tuple):
#                         match = match[0]
#                     if 14 <= len(match) <= 18:  # Typical UDI length range
#                         return match.strip()
        
#         return None
#     except Exception as e:
#         st.error(f"UDI Extraction Error: {str(e)}")
#         return None
