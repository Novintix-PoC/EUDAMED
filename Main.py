...
st.markdown("""
<nav class="navbar fixed-top navbar-expand-lg navbar-dark" style="background-color: #064462;">
    <div class="container-fluid d-flex justify-content-between align-items-center">
        <h3 class="m-0">EUDAMED</h3>
        <img src="https://novintix.com/wp-content/uploads/elementor/thumbs/Logo-NovaByte-white-qhoy1tkmiw3ti8f1wxmbj06hqh2di9ko38y6y54hs0.png" alt="Logo" style="height: 40px;">
    </div>
</nav>
""", unsafe_allow_html=True)
...
st.markdown("""
    <style>
        ...
        [data-testid="stVerticalBlock"] {padding-top:10px}  
        ...
        * {
        ...
        [data-testid="stBaseButton-secondary"]:hover{
            background-color: rgba(244, 163, 3, 0.2);    
        }
    
        .st-key-file_uploader_container{
            # border: 2px solid #000000;
        ...
        [data-testid="stBaseButton-secondary"]:hover{
            background-color: rgba(244, 163, 3, 0.5) !important;  
            overflow:hidden;  
            padding:5px;
        }
    </style>
...
def Main():
    outer_col = st.columns([1, 6, 1])
    with outer_col[1]:
        with st.container(key="file_uploader_container"):

            icon1, icon2 = st.columns([1, 25])
            with icon1:
                st.image("https://cdn.iconscout.com/icon/free/png-512/free-upload-icon-download-in-svg-png-gif-file-formats--micons-vol-1-pack-user-interface-icons-432710.png?f=webp&w=512", width=50)
            with icon2:
                st.markdown("<h4>Upload Yours Files</h4>",unsafe_allow_html=True)
            ...
            if uploaded_files:
                ...
                if "processing_type" not in st.session_state:
                    ...
                    with col1:
                        if st.button("MDR", use_container_width=True):
                            st.session_state.processing_type = "MDR"
                            st.rerun()
                    with col2:
                        if st.button("IVDR", use_container_width=True):
                ...
                if "processing_type" in st.session_state and "processing" not in st.session_state:
                    st.write(f"Select {st.session_state.processing_type} subcategory:")

                    if st.session_state.processing_type == "MDR":
                        col1, col2 = st.columns(2)
                        with col1:
                            if st.button("MDR Device", use_container_width=True):
                                st.session_state.processing = "MDR_Device"
                                st.rerun()
                        with col2:
                            if st.button("MDR System", use_container_width=True):
                                st.session_state.processing = "MDR_System"
                                st.rerun()
                    elif st.session_state.processing_type == "IVDR":
                        if st.button("IVDR Device", use_container_width=True):
